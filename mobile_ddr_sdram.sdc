...
set pins(addrcmd) { RAM_A0 RAM_A1 RAM_A2 RAM_A3 RAM_A4 RAM_A5 RAM_A6 RAM_A7 \
                    RAM_A8 RAM_A9 RAM_A10 RAM_A11 RAM_A12 \
                    RAM_BA0 RAM_BA1 \
                    RAM_CS_N RAM_CAS_N RAM_RAS_N RAM_WS_N RAM_CKE }

set pins(dqm)     { RAM_LDM RAM_UDM }

set pins(ck_p)    { RAM_CK_P }
set pins(ck_n)    { RAM_CK_N }

set dqgroup_0   "{RAM_LDQS} { RAM_D0 RAM_D1 RAM_D2 RAM_D3 RAM_D4 RAM_D5 RAM_D6 RAM_D7 }"
set dqgroup_1   "{RAM_UDQS} { RAM_D8 RAM_D9 RAM_D10 RAM_D11 RAM_D12 RAM_D13 RAM_D14 RAM_D15 }"
...
#MK:  Already defined in BeMicro_MK.sdc
#MK:create_clock -name sdram_base_clock -period $pll_ref_clk [get_ports your_clock_pin]

# Derive PLL clocks
#MK:  Already called in BeMicro_MK.sdc
#MK:derive_pll_clocks
...
set system_clk       {nios2_bemicro_sopc_inst|the_pll|sd1|pll7|clk[0]}
set dma_clk       {nios2_bemicro_sopc_inst|the_pll|sd1|pll7|clk[1]}
set sdram_clk        {the_M_DDR_PLL_inst|altpll_component|auto_generated|pll1|clk[0]}
set sdram_write_clk  {the_M_DDR_PLL_inst|altpll_component|auto_generated|pll1|clk[1]}
#######################################################
...
set_false_path -from [get_clocks $dma_clk] -to [get_clocks $sdram_clk]
set_false_path -from [get_clocks $sdram_clk] -to  [get_clocks $dma_clk]

set_false_path -from [get_clocks $dma_clk] -to [get_clocks $sdram_write_clk]
set_false_path -from [get_clocks $sdram_write_clk] -to  [get_clocks $dma_clk]
...
foreach dqgroup $pins(dqgroup) {
    ...
    set_false_path -from [get_clocks ${dqspin}_out] -to [get_clocks $dma_clk]
    set_false_path -from [get_clocks $dma_clk] -to [get_clocks ${dqspin}_out]
...
#    set_false_path -from port_clk -to [get_clocks _in]
    ...
    set_false_path -from [get_keepers {*|mtx_sdram_avalon_burst:*|sdram_reset*}] -to ${dqspin}_in

}

# Set false paths and multicycles for DDR same edge capture, must be done after all clocks are created
foreach dqgroup $pins(dqgroup) {
    ...
    # On same edge capture DDR interface we need a multicycle of 0 on the setup
    set_multicycle_path -setup -end -rise_from [get_clocks v_*_in] -rise_to [get_clocks ${dqspin}_in] 0
    set_multicycle_path -setup -end -fall_from [get_clocks v_*_in] -fall_to [get_clocks ${dqspin}_in] 0


}

# exclusive grouping for input and output
set_clock_groups -exclusive -group [get_clocks {*DQS_in}] -group [get_clocks {*DQS_out}]
...
set_false_path -from [get_keepers {*|mtx_sdram_avalon_burst:*|sdram_in_fifo_write_pointer*}] -to [get_keepers {*|mtx_sdram_avalon_burst:*|av_sdram_fifo_write_pointer_gray_sync*}]
set_false_path -from [get_keepers {*|mtx_sdram_avalon_burst:*|sdram_in_fifo_write_pointer*}] -to [get_keepers {*|mtx_sdram_avalon_burst:*|sdram_fifo_write_pointer_gray_sync*}]
set_false_path -from [get_keepers {*|mtx_sdram_avalon_burst:*|sdram_reset}] -to [get_keepers {*|mtx_sdram_avalon_burst:*|sdram_in_fifo_write_pointer*}]
set_false_path -from [get_keepers {*|mtx_sdram_avalon_burst:*|sdram_reset}] -to [get_keepers {*|mtx_sdram_avalon_burst:*|sdram_lsp_write_delayed*}]
...
set_false_path -from [get_keepers {*|mtx_sdram_controller:*|mtx_sdram_controller_scheduler:scheduler|sdram_select[*]}] -to [get_keepers {*|mtx_sdram_controller:*|sdram_data_to_sdram[*]}]
set_false_path -from [get_keepers {*|mtx_sdram_controller:*|mtx_sdram_controller_scheduler:scheduler|sdram_select[*]}] -to [get_keepers {*|mtx_sdram_controller:*|sdram_data_mask_to_sdram[*]}]
set_false_path -from [get_keepers {*|mtx_sdram_controller:*|lsp_ddr_burst_counter_run[*]}] -to [get_keepers {*|mtx_sdram_controller:*|sdram_lsp_ddr_burst_counter_run_sync}]
